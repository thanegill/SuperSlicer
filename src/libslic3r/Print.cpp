...
//    if (m_objects_temp_gcode.expired()) 
//        return ConstPrintObjectPtrsAdaptor(&m_objects);
//    else 
//        return ConstPrintObjectPtrsAdaptor(&*m_objects_temp_gcode.lock());
//}

PrintObject* Print::get_object(size_t idx) {
    if (auto ptr = m_objects_temp_gcode.lock(); !ptr) 
        return const_cast<PrintObject*>(m_objects[idx]);
    else 
        return const_cast<PrintObject*>((*ptr)[idx]);
}

const PrintObject* Print::get_object(size_t idx) const {
    return objects()[idx];
    if (auto ptr = m_objects_temp_gcode.lock(); !ptr)
...
const PrintObject* Print::get_object(ObjectID object_id) const {
    if (m_objects_temp_gcode.expired()) {
        ...
        auto objects = m_objects_temp_gcode.lock();
...
PrintStub::PrintStub(const Print& print, PrintObjectPtrs& objects) {
    //copy evrything

    this->m_config = print.m_config;
    this->m_default_object_config = print.m_default_object_config;
    this->m_default_region_config = print.m_default_region_config;
    this->m_objects = objects;
    this->m_objects_temp_gcode = print.m_objects_temp_gcode;
    this->m_print_regions = print.m_print_regions;
    ...
    this->m_first_layer_convex_hull = print.m_first_layer_convex_hull;
    this->m_skirt_convex_hull = print.m_skirt_convex_hull;

    // Following section will be consumed by the GCodeGenerator.
    this->m_tool_ordering = print.m_tool_ordering;
    ...
    //printbase
    this->m_model = print.m_model;
    this->m_full_print_config = print.m_full_print_config;
    this->m_physical_printer_config = print.m_physical_printer_config;
    this->m_placeholder_parser = print.m_placeholder_parser;

    // Callback to be evoked regularly to update state of the UI thread.
    this->m_status_callback = print.m_status_callback;

    //for gui status update
    this->m_last_status_update = print.m_last_status_update;
    this->m_last_status_percent = print.m_last_status_percent;

}
PrintStub::~PrintStub() {

    //let ~print delete objects (these are new)
    //this->m_objects.clear();
    this->m_print_regions.clear();
    //let m_model.clear_objects(); as it's a deep copy
}
void PrintStub::reset_tool_ordering() {
    m_wipe_tower_data.clear();
    m_tool_ordering.clear();
    if (this->has_wipe_tower()) {
        //this->set_status(45, L("Generating wipe tower"));
